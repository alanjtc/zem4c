class ZCL_EM4C_XML_BASE_SD_GENERATOR definition
  public
...
CLASS ZCL_EM4C_XML_BASE_SD_GENERATOR IMPLEMENTATION.
...
* - END OF Harold JTC 15.04.2025 09:04:33
    ...
    if rs_data-ecf-encabezado-totales-montototal < 250000.
...
*      ls_item-cantidaditem         = ls_vbrp-fkimg.
      ...
      ls_item-preciounitarioitem   = |{ ls_vbrp-kzwi1 / ls_vbrp-fkimg decimals = 2 }|.
...
*                                       next lv_sum = abs( ls_konv-kbetr ) ).
      ...
      if is_data-invoice-s_vbak-augru = 'Z03'.

        ls_item-montoitem = lc_cero.
        ls_item-preciounitarioitem = lc_cero.
...
* - END OF Harold JTC 15.04.2025 09:24:28
  ...
  method map_receiver.
    ...
    if rs_data-rnccomprador is initial.

      rs_data-rnccomprador = is_data-receptor-s_kna1-stcd2.
...
*    rs_data-montototal    = is_data-invoice-s_vbrk-netwr.
    ...
    if is_data-invoice-s_vbak-augru = 'Z03'.
      ...
      rs_data-montogravadoi1 = lc_cero.
      rs_data-montogravadototal = lc_cero.
      rs_data-montoexento = lc_cero.
      rs_data-totalitbis = lc_cero.
      rs_data-totalitbis1 = lc_cero.
